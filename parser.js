...
function parseBlock(lines, index, defaultSubject) {
  ...
  for (const original of body) {
    ...
    if (option) {
      ...
      const hasMark = MARK_RE.test(text) || MARK_RE.test(line.slice(0, prefixLength));
    ...
    if (currentOption && !isQuestionStructure(line) && !questionStart(line)) {
      if (MARK_RE.test(line)) answer = currentOption;
...
function stripMarks(text) {
  return String(text || '').replace(MARKS_GLOBAL, '').replace(/\s+\*\s*$/u, '').replace(/\s{2,}/g, ' ').trim();
}
