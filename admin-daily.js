import {
  ...
  show,
  esc
} from './app.js?v=20260729-parser-fix-v4';

import * as Parser from './parser.js?v=20260729-parser-fix-v4';
...
function exportSmartAnalyticsReport() {
  ...
  const lines = [
    'KSR EXAM OS+ · PHASE 5 STEP 2 SMART ANALYTICS',
...
function exportHealthReport(allQuestions, health, issues) {
  const lines = [
    'KSR EXAM OS+ - PHASE 4 PARSER HEALTH REPORT',
    `Generated: ${new Date().toLocaleString()}`,
...
function exportVersionSummary() {
  ...
  const text = `KSR EXAM OS+ — VERSION HISTORY\nExam ID: ${examId}\nGenerated: ${new Date().toLocaleString('en-IN')}\n\n${box.innerText}`;
...
function validateBackupPayload(payload) {
  if (!payload || payload.format !== 'KSR_EXAMOS_BACKUP') return 'ఇది valid KSR EXAM OS+ backup file కాదు.';
